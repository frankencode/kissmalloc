...
static_assert(sizeof(bucket_t) <= KISSMALLOC_GRANULARITY, "The size of the bucket header must not exceed KISSMALLOC_GRANULARITY bytes");
...
void *operator new(std::size_t size)
{
    ...
    #endif
    #endif
    if (size && !data) throw std::bad_alloc{};
    return data;
}
